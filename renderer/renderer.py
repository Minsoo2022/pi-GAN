...
class Renderer():
    def __init__(self, cfgs):
        ...
        self.device = cfgs.get('device', 'cpu')
        self.image_size = cfgs.get('img_size', 64)
        self.min_depth = cfgs.get('ray_start', 0.9)
        self.max_depth = cfgs.get('ray_end', 1.1)
        self.rot_center_depth = cfgs.get('rot_center_depth', (self.min_depth + self.max_depth) / 2)
        self.fov = cfgs.get('fov', 10)
        self.tex_cube_size = cfgs.get('tex_cube_size', 2)
        self.renderer_min_depth = cfgs.get('renderer_min_depth', 0.1)
    ...
    def render_yaw(self, im, depth, v_before=None, v_after=None, rotations=None, maxr=90, nsample=9, crop_mesh=None):
        ...
        for i, ri in enumerate(rotations):
            ...
            warped_depth = self.renderer.render_depth(grid_3d_i, faces)
            margin = (self.max_depth - self.min_depth) / 2
            warped_depth = warped_depth.clamp(min=self.min_depth - margin, max=self.max_depth + margin)
            im_trans += [warped_images]
            depth_trans += [warped_depth.unsqueeze(0)]
        return torch.stack(im_trans, 1), torch.stack(depth_trans, 1)  # b x t x c x h x w

    def render_h_v(self, im, depth, warping_pose, v_before=None, v_after=None, rotations=None, maxr=90, nsample=9,
                   crop_mesh=None):
        b, c, h, w = im.shape
        grid_3d = self.depth_to_3d_grid(depth)
        ...
        ri = torch.cat((warping_pose, torch.zeros((warping_pose.shape[0], 1)).to(warping_pose.device)), dim=1)
        # ri = torch.FloatTensor([0, ri, 0]).to(im.device).view(b, 3)
        rot_mat_i, _ = get_transform_matrices(ri)
        grid_3d_i = self.rotate_pts(grid_3d, rot_mat_i)
        ...
        faces = get_face_idx(b, h, w).to(im.device)
        textures = get_textures_from_im(im, tx_size=self.tex_cube_size)
        warped_images = self.renderer.render_rgb(grid_3d_i, faces, textures.type(torch.float32)).clamp(min=-1., max=1.)
        warped_depth = self.renderer.render_depth(grid_3d_i, faces)
        margin = (self.max_depth - self.min_depth) / 2
        warped_depth = warped_depth.clamp(min=self.min_depth - margin, max=self.max_depth + margin)
        return warped_images, warped_depth.unsqueeze(1)  # b x t x c x h x w